 ...
 */
export function shouldAutoApproveMcpElicitation(
  params: unknown,
  ...
  if (!meta || typeof meta !== "object" || Array.isArray(meta)
      || (meta as Record<string, unknown>).codex_approval_kind !== "mcp_tool_call") return false;
  try {
    return isManagedServer(request.serverName);
  } catch {
    // Configuration identity and parsing are part of the proof. Any failure
    // keeps the request on the normal browser-approval path.
    return false;
  }
}
