...
**v1.1.0 · 安全与稳定性修复版**：加强管理权限隔离、后台进程回收、部署路径保护与断线发送恢复。旧版用户请先看[升级与迁移](#升级与迁移)；完整变更和已知限制见 [CHANGELOG](CHANGELOG.md)。
...
- **附件与执行过程**：发送文件或图片附件，查看回复、命令、文件修改和工具调用；需要批准或填写信息时，在交互区处理。底部抽屉提供 diff 汇总与网页终端。
...
管理页会保存并核对最近一次操作的编号和结果；“配置完成”不等于“重启完成”。若显示结果未知，请核对配置与服务状态，不要重复点击；后台进程尚未确认停止时会继续阻止新任务。
...
**v1.0.1 → v1.1.0 是一次部署结构迁移。** 需要重新注册服务，建立独立网关账号、控制目录和进程启动器；仅替换 `dist` 不够。请先结束任务、停机并备份，使用新版脚本执行修复重装。不要依赖旧版更新器完成首次迁移。具体步骤见[首次迁移到 v1.1.0](deploy/README.md#首次迁移到-v110)。
...
- **网页终端不继承输入框中当前回合的权限选择。** 它通过 Codex `command/exec` 运行，未指定的沙箱策略取决于 Codex 配置；应按 worker 账号可用的 shell 权限管理风险。项目列表只是工作目录选择，不是权限边界。
...
`protocol/` 是固定 Codex 版本生成并提交的协议类型。CI 检查类型、测试、构建、ShellCheck、依赖与敏感文件，另有 CodeQL 与上游兼容性检查；不会自动升级 Codex。私有开发目录不得提交，已有目录应自行备份或迁移，不要当作构建缓存删除。

## 已知限制

v1.1.0 并非“零缺陷”保证。低优先级问题仍包括断线时审批项显示、并发历史图片加载、失败回合的历史展示，以及多标签页的部分状态同步。遇到结果未知时，先核对服务日志与会话历史，**不要重复发送可能已执行的操作**。详细症状、运维限制和验证边界见 [CHANGELOG](CHANGELOG.md#已知限制与验证边界)。
